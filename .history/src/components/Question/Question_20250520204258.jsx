...
function Question2(props) {
	...
	const soundRef = useRef(null) // Add this ref to keep track of the sound instance
	useEffect(() => {
	...
	useEffect(() => {
		...
		if (
			...
			if (props.url) {
				// If there's a previous sound, stop it
				if (soundRef.current) {
		...
		// Cleanup function to stop sound when component unmounts or changes
		return () => {
			if (soundRef.current) {
				soundRef.current.stop()
			}
