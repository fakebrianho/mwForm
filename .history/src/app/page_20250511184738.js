...
// Create a CameraAnimation component using GSAP
function CameraAnimation({ isLoaded }) {
	const { camera } = useThree()
	const gsapRef = useRef()

	useEffect(() => {
		if (isLoaded && !gsapRef.current) {
			// Initially position camera far away
			camera.position.set(0, 0.5, 20)

			// Create the animation
			gsapRef.current = gsap.to(camera.position, {
				z: 5, // Target z position
				duration: 3, // Animation duration in seconds
				ease: 'power2.out', // Smooth easing
				delay: 0.5, // Start after the fade begins
			})
		}
	...
	useEffect(() => {
		if (modelsLoaded && fadeOverlayRef.current) {
			...
			tl.to(fadeOverlayRef.current, {
				opacity: 0,
				duration: 2, // Fade duration in seconds
				ease: 'power2.inOut',
				delay: 1, // Wait a second before starting fade
			})
	...
	return (
		<div className={styles.page}>
			<div id='canvas-container'>
				...
				>
					<CameraAnimation isLoaded={loadingComplete} />
					<mesh>
						<OrbitControls
							enableZoom={false}
							enableDamping={true}
