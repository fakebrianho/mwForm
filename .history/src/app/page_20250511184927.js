...
export default function Home() {
	...
	// Improved fade transition with GSAP
	useEffect(() => {
		if (modelsLoaded) {
			...
			const timer = setTimeout(() => {
				...
				if (!fadeRef.current) {
					...
					fadeRef.current = overlay
				}

				// Fade out animation
				gsap.to(fadeRef.current, {
					opacity: 0,
					duration: 1.5, // Slightly faster fade
					ease: 'power2.inOut',
					onComplete: () => {
						setLoadingComplete(true)
						// Remove the overlay from DOM when done
						if (fadeRef.current && fadeRef.current.parentNode) {
							fadeRef.current.parentNode.removeChild(
								fadeRef.current
							)
			...
			return () => {
				clearTimeout(timer)
				// Clean up overlay if component unmounts during transition
				if (fadeRef.current && fadeRef.current.parentNode) {
					fadeRef.current.parentNode.removeChild(fadeRef.current)
					fadeRef.current = null
				}
			}
		}
