...
export default function Home() {
	...
	// Add this new function to handle stage advancement
	const handleNextStage = (currentStage, value) => {
		// Check if the current field is filled before allowing advancement
		if (!value.trim()) {
			// Create a temporary error for this field
			setFormErrors({
				...formErrors,
				[`answer${currentStage}`]: `This field is required`,
			})
			return // Prevent advancing to next stage
		}

		// Clear any error for this field and advance stage
		setFormErrors({
			...formErrors,
			[`answer${currentStage}`]: null,
		})
	...
	useEffect(() => {
		if (stage === 8) {
			const errors = {}
			if (!answer1.trim()) errors.answer1 = 'Shop name is required'
			if (!answer2.trim()) errors.answer2 = 'Instagram handle is required'
			if (!answer3.trim()) errors.answer3 = 'City is required'
			if (!answer5.trim()) errors.answer5 = 'Shop fee is required'
			if (!answer6.trim()) errors.answer6 = 'Contact info is required'
			if (!answer7.trim()) errors.answer7 = 'Email is required'
			...
			if (Object.keys(errors).length === 0) {
				...
				// Return to the first stage with an error
				for (let i = 1; i <= 7; i++) {
					if (errors[`answer${i}`]) {
						setStage(i)
						break
